...
function Stage({
  ...
  fit = 'contain',   // 'contain' (letterbox) | 'cover' (fill viewport, may crop)
  children,
}) {
  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      const barH = fit === 'cover' ? 0 : 44; // playback bar height
      const sw = el.clientWidth / width, sh = (el.clientHeight - barH) / height;
